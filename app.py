...
if menu == "Home":
    ...
    # Chart 6: Receiver Type Distribution
    ...
    with col1:
        selected_city = st.selectbox("Filter by City", ["All"] + sorted(food_listings['Location'].unique().tolist()))
    with col2:
        selected_food_type = st.selectbox("Filter by Food Type", ["All"] + sorted(food_listings['Food_Type'].unique().tolist()))
    with col3:
        selected_meal_type = st.selectbox("Filter by Meal Type", ["All"] + sorted(food_listings['Meal_Type'].unique().tolist()))
    ...
    st.info("CRUD operations work on local MySQL database. For cloud demo, data is loaded from CSV files.")
    ...
    if operation == "Add Food Listing":
        ...
        if st.button("Add Listing"):
            st.success(f"'{food_name}' listing added successfully! (Demo mode)")
        ...
        if st.button("Update"):
            st.success(f"Food ID {food_id} updated successfully! (Demo mode)")
        ...
        if st.button("Delete"):
            st.success(f"Food ID {food_id} deleted successfully! (Demo mode)")